...
def sort_group():
    new_dict = sort_model()
    for key in new_dict:
        cmds.group(em=True, n='mao_faceNum_'+str(key), parent='mao')
        cmds.parent( new_dict[key], 'mao_faceNum_'+str(key) )
...
def unit():                    
    ...
    group_name = cmds.textField('group_name',q=True,text=True)
    del_line(group_name,sel_list)
...
def ui():
    ...
    cmds.columnLayout()
    cmds.button('Create Group',c='sort_group()')
    ...
    cmds.text('group_name:')
    cmds.textField('group_name')
